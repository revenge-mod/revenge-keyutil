...
import { toUint8Array } from './src/buffer'
...
program
    ...
    .action(({ name, expires, privateKeyPath, publicKeyPath }) => {
        ...
        writeFileSync(privateKeyPath, toUint8Array(privateKey))
        writeFileSync(publicKeyPath, toUint8Array(publicKey))
...
program
    ...
    .action((filePath, { signature: signatureFilePath, Pr: keyPath }) => {
        ...
        const key = readRevengeKey(readFileSync(keyPath))

        if (!key.isPrivate()) throw new Error('Key must be a private key')

        const signature = key.sign(hash)
        writeFileSync(signatureFilePath, toUint8Array(signature))
...
program
    ...
    .action((filePath, { signature: signatureFilePath, Pb: keyPath }) => {
        const hash = sha512(readFileSync(filePath))
        const signature = readRevengeSignature(readFileSync(signatureFilePath))
        const key = readRevengeKey(readFileSync(keyPath))

        if (!key.isPublic()) throw new Error('Key must be a public key')

        const isValid = key.verify(signature.signature, hash)
        console.log(`Signature is${isValid ? '' : ' NOT'} valid`)
    })
...
program
    ...
    .action(path => {
        ...
        const signature = readRevengeSignature(file)
...
program
    ...
    .action((publicKeyPath, { Pr }) => {
        ...
        const publicKey = readRevengeKey(publicKeyFile)
        const privateKey = readRevengeKey(privateKeyFile)
...
program
    ...
    .action((path, cPP, { longSignatures }) => {
        const file = readFileSync(path)
        const key = readRevengeKey(file)
        const cKey = readRevengeKey(readFileSync(cPP))
